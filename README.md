...
Copy the snapshot content (except `testcase-snapshot:`) and append to a test-suite file, [example-suite.yml](https://raw.github.com/opwire/sample-cmdline-node/master/tests/example-suite.yml) for example:

```yaml
testcases:
- title: The first testcase (generated by opwire-testa)
  uuid: 3e4faf4f-65ba-11e9-86de-2047475d3f63
  request:
    method: POST
...
### Step 5. Verify the updated test

Run testing with the following command:
...
```
opwire-testa run --test-dirs=${PWD}/tests
```
